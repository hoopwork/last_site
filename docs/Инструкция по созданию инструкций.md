...
ТЕКСТ ТЕКСТ ТЕКСТ. Больше шрифт (6 размер) и толщина 300
{: .fs-6 .fw-300 } 

Еще одна странная стилистика текста
{: .text-gamma }

Еще прикольная стилистика
МОЙ ТЕКСТ МОНО
...
и с расстоянием между кнопками в ряд
...
{: } - так добавляется модифицированный стиль



### Моя кастомизация
делаю свой стиль (тему) в _sass/color_schemes/my_style.scss. В нем меняю переменные касаемо цветов, шрифтов, отступов (они перечислены в _sass/support/_variables.scss, сам файл возбраняется редактировать).
Моя тема накладывается на тему light, т.к. она подефолтная.
Если хочу накладывать на Dark, то в начале файла темы делаю @import "./color_schemes/dark"
Далее указываю свою тему в _config.yml


Для добавления новых переменных (а не переопределения старых) используется файл: _sass/custom/setup.scss
Он импортируется после того как определены стандартные переменные темы, но до создания готовых CSS. Короче обрабатывается почти в конце.
Тут же переопределяются CSS классы, можно сделать полностью свой стиль, не основанный на стандартном?

Через папку _includes я могу переопределить чето. Видимо мои файлы заменят оригинальные:
toc_heading_custom.html - файл со списком TOC: отображающий дочерние страницы
_includes/footer_custom.html - файл с футером низа страницы, который возбраняется. Не использовать его
_includes/head_custom.html - файл включается между тегов head, поэтому используется для мета информации или js-скриптов. Здесь же ссылка на фавикон
_includes/header_custom.html - файл содержит место между поиском и верхними правыми ссылками
_includes/nav_footer_custom.html - файл содержит футер под боковым меню, где написано Just The Docs
_includes/search_placeholder_custom.html - файл содержит текст местозаполнителя который в панели поиска


И основные компоненты страницы, включенные в файл _layouts/default.html.
